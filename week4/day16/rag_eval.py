...
"""
    ...
    for attempt in range(5):
        try:
            ...
            if "429" in str(e) or "rate_limit_exceeded" in str(e).lower() or "rate limit" in str(e).lower():
                print(f"Rate limit hit in ask_llm, retrying in 4 seconds (Attempt {attempt+1}/5)...")
                time.sleep(4)
            else:
                raise e

    raise Exception("Rate limit exceeded in ask_llm after multiple retries")
...
def llm_judge(prompt):
    ...
    for attempt in range(5):
        try:
            ...
            if "429" in str(e) or "rate_limit_exceeded" in str(e).lower() or "rate limit" in str(e).lower():
                print(f"Rate limit hit in llm_judge, retrying in 4 seconds (Attempt {attempt+1}/5)...")
                time.sleep(4)
            else:
                raise e

    raise Exception("Rate limit exceeded in llm_judge after multiple retries")
...
def run_evaluation():
    ...
    for test in golden_dataset:
        ...
        if precision < 0.7:
            ...
            print(
                "[FAIL] Retrieval is returning irrelevant chunks."
            )
        ...
        if recall < 0.7:
            ...
            print(
                "[FAIL] Retrieval is missing important information."
            )
        ...
        if faithfulness < 0.7:
            ...
            print(
                "[FAIL] Answer is not properly grounded."
            )
        ...
        if relevancy < 0.7:
            ...
            print(
                "[FAIL] Answer is not sufficiently relevant."
            )
        ...
        if correctness < 0.7:
            ...
            print(
                "[FAIL] Answer is not sufficiently correct."
            )
