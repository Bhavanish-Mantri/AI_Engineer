...
client = instructor.from_groq(Groq(api_key=my_api_key), mode=instructor.Mode.JSON)
...
CRITICAL: You must return ONLY the raw JSON object matching the schema. Do NOT include any preamble, introduction, markdown wrapping, thought process, or explanations.
...
def parse_resume(resume_text):
    ...
    CRITICAL: You must output ONLY the JSON object. Do NOT include any preamble, introduction, markdown codeblocks (like ```json), thought process, or explanations. Do NOT summarize or reprint the resume text.
